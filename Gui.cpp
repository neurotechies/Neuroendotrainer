...
using std::string;
    ...
    Gui::Gui()
	{
        m_window_name = "Neuro-Endo-Trainer";
	}
	...
	void Gui::init()
	{
        ...
        cvResizeWindow(m_window_name.c_str(), 750, 600);
        cvMoveWindow(m_window_name.c_str(), 4, 4);
	}
	...
	int getBBFromUser(IplImage *img, CvRect &rect, Gui *gui, const string &message)
	{
        ...
        cvResizeWindow(window_name.c_str(), 750, 600);
	...
	int getRingthresholdFromUser(IplImage *img, Gui *gui, string message, int RingThreshVal, Rect r, cv::Mat &fgMaskRing)
	{
		...
		cvNamedWindow(window_name.c_str(), WINDOW_NORMAL);
        cvResizeWindow(window_name.c_str(), 750, 600);
        cvMoveWindow(window_name.c_str(), 4, 4);
